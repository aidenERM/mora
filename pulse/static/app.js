...
async function loadAuthenticatedState(options = {}) {
  ...
  const [events, preferences, integrations, plans] = await Promise.all([api("/api/events?limit=100"), api("/api/preferences"), api("/api/integrations"), api("/api/plans?limit=20")]);
...
document.addEventListener("click", async (event) => {
  ...
  try {
    ...
    if (action === "approve-plan") { const button = event.target.closest("[data-plan-id]"); const planId = button?.dataset.planId; const target = button?.dataset.target || "google"; const result = await api(`/api/plans/${encodeURIComponent(planId)}/approve`, { method: "POST", body: JSON.stringify({ target }) }); await loadAuthenticatedState(); renderHome(); showToast(result.queued ? "approved · waiting for iPhone" : result.ok ? "added to Google Calendar" : "calendar action failed", !result.ok); }
    ...
    if (action === "shortcut-test-action") {
      const result = await api("/api/shortcut/test-action", { method: "POST", body: JSON.stringify({ action_type: event.target.closest("[data-test-type]")?.dataset.testType }) });
      showToast(result.action ? "test action queued" : "test action failed", !result.action);
      await loadAuthenticatedState(); renderAppleSetup();
    }
