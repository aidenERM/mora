...
The worker keeps a 15-minute cadence for direct watchers: La Ceja weather, the official Apple Newsroom feed, the official Call of Duty blog URL, a Colombia RSS search feed, configured GitHub repositories, and optional custom RSS/URL watchers. The first successful source check seeds history without sending a notification for every old item. Weather looks 24 hours ahead by default and can create separate, deduplicated alerts for rain, thunderstorms, heat, cold, strong wind, and fog. Each alert includes the expected time, temperature/feels-like values, precipitation probability/amount, and wind detail where relevant. Thresholds are configurable in `.env.example`.
